...
import getRefrestToken from '../middlewares/getRefreshToken.js';
...
router.post('/login', getLoginUser, verifyLoginUser, insertUserSession, (req, res, next) => {
    /**
     ...
     *   deviceID: <string> (Required) - The device of the user.
    ...
    if (model.user.isNeedsOTP) req.url = '/login/otp';
    else req.url = '/login/none-otp';

    return next();
});

router.post('/login/none-otp', getUserSession, getRefrestToken, createRefreshToken, createAccessToken, setCookies, (req, res) => {
    /**
     * Destroy the session
     * 
     * Fields:
...
router.post('/login/otp', getUserSession, createOTP, sendOTP, createConfirmToken(UKEY), insertUserSession, async (req, res) => {
    const model = getModelService(req);
...
router.post('/login/otp/verify', getUserSession, verifyOTP, getRefrestToken, createRefreshToken, createAccessToken, setCookies, async (req, res) => {
    /**
     * This route handler expects a POST request body with the following structure:
     * 
     * {
...
router.get('/login/otp/resend', verifyConfirmToken(UKEY), getUserFromID, createOTP, sendOTP, insertUserSession, async (req, res) => {
    /**
