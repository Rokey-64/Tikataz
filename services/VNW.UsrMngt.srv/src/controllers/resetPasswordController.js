 ...
 * @returns
 */
router.post('/reset-password/recover/new-otp', inspector, createOTP, sendOTP, createConfirmToken(UKEY), insertUserSession, async (req, res, next) => {
    /**
     ...
     *   deviceID: <string> (Required) - The device ID of the user.
...
router.get('/reset-password/recover/resend-otp', verifyConfirmToken(UKEY), getUserFromID, createOTP, sendOTP, insertUserSession, async (req, res, next) => {
    /**
     * This route handler expects a POST request body with the following structure:
     * 
     * {
 ...
 */
router.post('/reset-password/recover/verify-otp', verifyConfirmToken(UKEY), getUserSession, verifyOTP, async (req, res, next) => {
    /**
     * This route handler expects a POST request body with the following structure:
     * 
     * {
 ...
 */
router.post('/reset-password/recover/', verifyConfirmToken(UKEY), checkNewPassword, resetPassword, removeRefreshTokens, async (req, res, next) => {
    ...
    try {
        await deleteRedisKey(`confirm-token:${payload?.userID}:${model?.deviceID}`);
    } catch (error) {
