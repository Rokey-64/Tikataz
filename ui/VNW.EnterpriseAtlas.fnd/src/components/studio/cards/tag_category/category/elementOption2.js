 ...
 */
const ElementOption2 = ({path, root, callback, title, options, explain, children}) => {

    const createElement = (option, index) => {
        const [key, value] = Object.entries(option)[0];
        return (
            <ElementOption key={index} title={value} callback={callback} path={`${path}.${key}`} root={root}/>
        );
    }
